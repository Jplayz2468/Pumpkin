...
impl BlockBehaviour for TwistingVinesBlock {
    ...
    // (TWISTING_VINES_PLANT) via GrowingPlantBodyBlock.java:68.
    fn is_valid_bonemeal_target(&self, args: BonemealArgs<'_>) -> bool {
        let Some(head_pos) = twisting_vines_head_pos(args.world, args.block, args.position) else {
            return false;
        };
        let growth_pos = head_pos.up();
        twisting_vines_can_grow_into(args.world.get_block(&growth_pos))
            && args.world.is_in_build_limit(growth_pos)
    }

    // GrowingPlantHeadBlock.java:125 performBonemeal, blocksToGrow via
    // NetherVines.getBlocksToGrowWhenBonemealed (TwistingVinesBlock.java:24).
    fn perform_bonemeal(&self, args: BonemealArgs<'_>) {
        ...
        let mut forward_pos = head_pos.up();
        for _ in 0..blocks_to_grow {
            if !twisting_vines_can_grow_into(args.world.get_block(&forward_pos))
            ...
            args.world.set_block_state(
                &forward_pos,
                new_props.to_state_id(&Block::TWISTING_VINES),
                BlockFlags::NOTIFY_ALL,
            );
            forward_pos = forward_pos.up();
...
fn twisting_vines_can_grow_into(block: &Block) -> bool {
    block == &Block::AIR
}
...
/// Resolves the TWISTING_VINES head position for either the head or the body
/// (TWISTING_VINES_PLANT), mirroring `BlockUtil.getTopConnectedBlock` (growth
/// direction UP for twisting vines).
fn twisting_vines_head_pos(
    world: &Arc<World>,
    block: &Block,
    position: &BlockPos,
) -> Option<BlockPos> {
    ...
    if block != &Block::TWISTING_VINES_PLANT {
        return None;
    }
    ...
    for _ in 0..max_steps {
        ...
        if found == &Block::TWISTING_VINES_PLANT {
            continue;
        }
        return if found == &Block::TWISTING_VINES {
            Some(current)
        } else {
            None
        };
