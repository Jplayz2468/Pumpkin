...
impl PotentSulfurBlockEntity {
    ...
    fn waiting_countdown_tick(&self, world: &Arc<World>, current_state: PotentSulfurState) {
        ...
        if self.waiting_countdown.load(Ordering::Relaxed) == 0 {
            ...
            let mut props =
                PotentSulfurLikeProperties::from_state_id(world.get_block_state(&self.position).id);
            ...
            if new_state == PotentSulfurState::Dormant {
                world.emit_game_event("block_deactivate", self.position.to_centered_f64());
            }
        }
    }
    ...
    fn launch_entity_tick(&self, world: &Arc<World>) {
        ...
        let max_speed_to_launch = 0.3f64 + f64::from(water_blocks) * 0.1;
        for entity in entities {
            ...
            if entity.get_player().is_some_and(|player| player.is_flying()) {
                continue;
            }
            ...
            if velocity.y < max_speed_to_launch {
                base_entity
                    .velocity
                    .store(Vector3::new(velocity.x, velocity.y + 0.2, velocity.z));
            }
        }
    }
}
...
fn find_noxious_gas_source_block(world: &Arc<World>, origin: &BlockPos) -> Option<BlockPos> {
    ...
    loop {
        ...
        if !is_water_source || (block != &Block::WATER && !is_geyser_passable_block(state, block, &pos))
        {
...
fn can_be_reached_by_noxious_gas(world: &Arc<World>, source: &BlockPos, eye_pos: Vector3<f64>) -> bool {
    ...
    let (below_fluid, below_fluid_state) = world.get_fluid_and_fluid_state(&below_block_pos);
